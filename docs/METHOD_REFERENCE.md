...
**190 methods** (178C/R/U/D + 12E) across **46 features**
...
- **`get_arp_inspection()`** — Read
  Returns: `validate_src_mac`, `validate_dst_mac`, `validate_ip`, `auto_disable`, `ports`, `vlans`
- **`set_arp_inspection()`** — Update
- **`set_arp_inspection_port()`** — Update
- **`set_arp_inspection_vlan()`** — Update
...
- **`get_interfaces()`** — Read, keyed by `name`
  Returns: `oper_status`, `admin_status`, `alias`, `speed`, `mtu`, `phys_address`, `autoneg_enabled`, `autoneg_supported`, `media_type`, `manual_config`, `cable_crossing`, `flow_control`, `link_trap`, `power_state`, `power_save`, `signal`, `track_name`
- **`get_interface_statistics()`** — Read, keyed by `name`
  Returns: `tx_errors`, `rx_errors`, `tx_discards`, `rx_discards`, `tx_octets`, `rx_octets`, `tx_unicast_packets`, `rx_unicast_packets`, `tx_multicast_packets`, `rx_multicast_packets`, `tx_broadcast_packets`, `rx_broadcast_packets`, `fragments`, `crc_errors`, `collisions`, `late_collisions`, `utilization`, `utilization_interval`, `utilization_alarm_lower`, `utilization_alarm_upper`, `utilization_alarm`
- **`set_interface_utilization()`** — Update
- **`clear_interface_statistics()`** — Update
...
- **`get_ipv6_neighbors()`** — Read, keyed by `ip`
  Returns: `interface`, `ip`, `mac`, `state`
- **`get_ipv6_neighbors_table()`** — Read, keyed by `ip`
  Returns: `interface`, `ip`, `mac`, `state`
...
- **`get_remote_auth()`** — Read
  Returns: `radius_enabled`, `radius_timeout`, `radius_retransmits`, `tacacs_accounting`, `ldap_enabled`, `radius_servers`, `ldap_servers`, `tacacs_servers`
...
- **`cli()`** — Execute (SSH); unbounded escape hatch (undeclared capability)
