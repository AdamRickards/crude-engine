...
| SSH get_lldp_neighbors remote-data sections/block parser | Schema NO_HOLE — CLI.json names `show lldp remote-data` (+ global/port); archive fixture + `local/archive/lldp.yaml` need `parser: sections` (block headers `Remote data, M/N - #port` + dotted field labels). Live `wire/ssh/lldp.yaml` write-only ("reads need block parser"). Live parsers only table/paired_rows/simple_table/regex — cannot multi-section list_append by local_port. Scalar `field:` invents single-record floor. `cli=show port` is ifindex key_map only. Prefer existing tools exhausted. Soft Rank1 exhausted | #333 get_lldp_neighbors | Soft Σ SSH fail continue (#334+); Engine sections/block parser / HITL when opened |
