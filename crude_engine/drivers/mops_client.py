...
Usage:
    ...
    with MOPSClient("192.0.2.10", "admin", "private") as m:
...
def _decode_hex_string(value):
    """Decode MOPS hex-encoded string to text. Returns bytes for binary data."""
    ...
    if all(len(p) == 2 for p in parts):
        try:
            ...
            # Check if it's likely binary (e.g. InetAddress)
            # Binary data often contains non-printable control characters
            # or is exactly 4 bytes (IPv4).
            decoded = raw.decode("utf-8", errors="strict")
            # If it's short and has low chars, keep as bytes
...
class MOPSClient:
    ...
    def _build_get_request(self, queries, source="running-config"):
        ...
        mib_nodes = {}
        for q in queries:
            mib_name, node_name, attrs = q[0], q[1], q[2]
