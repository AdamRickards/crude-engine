...
DEFAULT_METHODS = ("get_facts", "get_mrp", "get_dns", "get_interfaces", "get_banner", "get_ntp", "get_syslog", "get_hidiscovery", "get_login_policy", "get_gvrp", "get_gmrp", "get_dai_global", "get_dhcp_snooping", "get_session_config", "get_snmp_config", "get_services", "get_rstp", "get_loop_protection", "get_vlan_ingress", "get_auto_disable_reasons", "get_snmp_information", "get_qos", "get_remote_auth", "get_system_info", "get_port_security", "get_rstp_port", "get_config_remote", "get_qos_mapping", "get_storm_control", "get_ip_restrict", "get_arp_inspection", "get_signal_contact", "get_config_status", "get_management_priority", "get_watchdog_status")
    ...
    for tok in tokens:
        if isinstance(cur, dict):
            if tok in cur:
                cur = cur[tok]
                continue
            # JSON gold keys are strings; Offline row keys may be ints.
            if isinstance(tok, str) and tok.isdigit():
                ik = int(tok)
                if ik in cur:
                    cur = cur[ik]
                    continue
            if not isinstance(tok, str) and str(tok) in cur:
                cur = cur[str(tok)]
                continue
            return None
        if isinstance(cur, list) and isinstance(tok, int) and 0 <= tok < len(cur):
            cur = cur[tok]
            continue
        return None
    return cur
